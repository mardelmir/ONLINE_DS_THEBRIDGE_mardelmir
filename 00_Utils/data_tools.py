...
def classify_by_cardinality(
    df, discrete_threshold=9, continuous_threshold=15, sort_ascending='origin', sugg_type=None, index_first=False
):
    """
    ...
    for col in df.columns:
        if (
            df[col].dtype == 'object'
            and df[col].str.contains(r'\w').any()
            and df_temp.at[col, '% Cardinality'] == 100.0
        ):
...
def categorical_numerical_test(df, target, alpha=0.05, significant_only=False):
    ...
    if significant_only:
        results_df = results_df.loc[results_df['significant'] is True]
...
def numerical_correlation_spearman(df, target, *, show_heatmap=True, annot=False, threshold=0, significant_only=False):
    ...
    if significant_only:
        spearman_df = spearman_df[spearman_df['significant'] is True]
...
add_space = lambda text: re.sub(r'(?<!^)(?=[A-Z])', ' ', text)  # noqa: E731
